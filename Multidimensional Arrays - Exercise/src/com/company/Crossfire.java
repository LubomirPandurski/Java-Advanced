...
public class Crossfire {
    ...
    public static int[] getElements(String input) {
        return Arrays
                .stream(input.split("[\\s]+"))
                .mapToInt(Integer::parseInt)
                .toArray();
    }
    private static void removeElement(int row, int col, int [][] matrix) {
    ...
    public static void main(String[] args) {
        ...
        while (!command.equals("Nuke it from orbit")){
            int[] numbers = getElements(command);
            int row = numbers[0];
            int col = numbers[1];
            ...
            for (int i = start; i <= end; i++) {
                if (isValid(i, col, matrix) && i != row) {
                    removeElement(i, col, matrix);
                }
            }
